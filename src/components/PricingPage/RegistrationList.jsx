...
// //         // if search matches phone → show real phone, else mask
// //         const showPhone = searchPhone && reg.phone.includes(searchPhone);

// //         return {
// //           ...reg,
// //           name: maskString(reg.name, 3),
// //           email: maskString(reg.email, 4),
// //           phone: showPhone ? reg.phone : maskString(reg.phone, 3),
// //           planName: reg.planName,
// //         };
// //       });
...
// //   useEffect(() => {
// //     fetchFromAPI(); // re-run whenever admin toggles or search changes
// //   }, [isAdmin, searchPhone]);
...
// //           className="w-full max-w-md px-4 py-2 rounded-lg border border-green-400 focus:outline-none focus:ring-2 focus:ring-green-500"
// //         />
// //       </div>
...
//       const filtered = merged.map((reg) => {
//         if (isAdmin || reg.email === userEmail) return reg;
//         const showPhone = searchPhone && reg.phone.includes(searchPhone);
//         return {
//           ...reg,
//           name: maskString(reg.name, 3),
...
//   // ✅ If searching → only show first match
//   const matchedUser = searchPhone
//     ? registrations.find((reg) => reg.phone.includes(searchPhone))
//     : null;

//   const currentData = matchedUser
//     ? [matchedUser]
//     : registrations.slice(
//         (currentPage - 1) * ITEMS_PER_PAGE,
//         currentPage * ITEMS_PER_PAGE
...
//       {/* Pagination → hide if searching */}
//       {!searchPhone && totalPages > 1 && (
//         <div className="flex justify-center mt-8 gap-2 flex-wrap">
//           {Array.from({ length: totalPages }, (_, i) => (
//             <button
//               key={i + 1}
...
export default function RegistrationList({ currentUser }) {
  ...
  const changeStatus = async (uid, status) => {
    ...
    try {
      ...
      // ✅ Modified: added reply_to so replies go to user
      await emailjs.send(
        SERVICE_ID,
        TEMPLATE_ID,
        {
